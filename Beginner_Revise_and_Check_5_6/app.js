...
let currentAudio=null,playToken=0,currentUtterance=null,currentAudioButton=null,currentAudioIdle='';
...
function resetAudioButton(){if(currentAudioButton){currentAudioButton.classList.remove('busy','error');currentAudioButton.textContent=currentAudioIdle||'▶ Прослушать'}currentAudioButton=null;currentAudioIdle=''}
function stopAudio(){playToken++;try{ttsAudio.pause();ttsAudio.removeAttribute('src');ttsAudio.load()}catch(e){}try{if(window.speechSynthesis)window.speechSynthesis.cancel()}catch(e){}resetAudioButton();currentAudio=null;currentUtterance=null}
...
async function playSequence(texts,btn,idle='▶ Прослушать',busyLabel='■ Стоп'){
 const wasBusy=!!btn?.classList.contains('busy');
 if(wasBusy){stopAudio();return}
 stopAudio();
 const token=++playToken;
 if(btn){currentAudioButton=btn;currentAudioIdle=idle;btn.classList.add('busy');btn.textContent=busyLabel}
 try{
  for(const t of texts){if(token!==playToken)return;await playChunk(t,token)}
  if(token===playToken)resetAudioButton();
 }catch(e){
  if(token===playToken){
   if(btn){btn.classList.remove('busy');btn.classList.add('error');btn.textContent='Аудио недоступно'}
   currentAudioButton=null;currentAudioIdle='';
   setTimeout(()=>{if(btn){btn.classList.remove('error');btn.textContent=idle}},2200)
  }
 }
}
function playText(text,btn,idle='▶ Прослушать',busyLabel='■ Стоп'){return playSequence(splitTTS(text),btn,idle,busyLabel)}
...
function passport(){
 ...
 app.innerHTML=shell(`${title(8,'Life Snapshot','Открой подсказки и найди человека')}<div class="blockBody"><div class="visualCard">${visualSlot('block8-snapshot.jpg','🔎','Life Snapshot','job · breakfast · routine','Lifestyle clues arranged on a modern desk')}</div><div class="questionCard snapshotQuestion"><div class="kicker">LIFE SNAPSHOT · PROFILE ${idx+1}/${data.passport.length}</div><div class="prompt">Who is it?</div><div class="subprompt">Открой дополнительные подсказки, если первой недостаточно.</div><div class="snapshotMain"><div class="clueStack">${item.clues.map((c,i)=>`<div class="clueCard ${i<revealed?'revealed':''}"><span>${i+1}</span><b>${i<revealed?esc(c):'••••••••'}</b>${i<revealed?`<button class="clueAudio" data-clue="${i}" aria-label="Прослушать подсказку">▶</button>`:''}</div>`).join('')}</div><div class="snapshotTools">${revealed<3&&!done?'<button class="revealBtn" id="revealBtn">Открыть ещё подсказку +</button>':'<span></span>'}</div><div class="profileOptions">${item.options.map(o=>`<button class="profileOption ${choice===o.name?'selected':''}" data-profile="${esc(o.name)}" ${done?'disabled':''}><span class="profileAvatar">${esc(o.name.slice(0,1))}</span><span class="profileCopy"><b>${esc(o.name)}</b><span>${esc(o.meta)}</span></span></button>`).join('')}</div><div class="snapshotActions"><button class="snapshotCheck" id="checkProfile" ${done?'disabled':''}>Проверить</button></div></div><div class="statusWrap"><div id="fb">${done?feedback('good',`Верно — ${esc(item.target)}!`):feedback('neutral','Выбери профиль')}</div>${miniProgress(sec,idx,data.passport.length)}</div></div></div><div class="footerActions"><div class="leftActions"></div><button class="nextBtn" id="next" ${done?'':'disabled'}>${idx===data.passport.length-1?'Результаты →':'Следующий профиль →'}</button></div>`);

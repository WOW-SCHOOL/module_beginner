...
function garageSort(){
 ...
 const mixedOrder=[9,1,6,3,11,4,0,8,2,10,5,7];
 if(!Array.isArray(state.sortOrder)||state.sortOrder.length!==total){state.sortOrder=mixedOrder.slice();save()}
 const remaining=state.sortOrder.map(i=>({...data.sort[i],i})).filter(x=>!state.sortPlaced[x.i]);
 const chips=remaining.map(x=>`<button class="sortChip ${state.sortSelected===x.i?'selected':''}" draggable="true" data-chip="${x.i}"><span>${esc(x.w)}</span></button>`).join('');
 const meta={
  'COLOURS':['🎨','Colours','red · blue · black · white'],
  'SIZE & SPEED':['⚡','Size & speed','big · small · fast · slow'],
  'PRICE & OPINION':['💬','Price & opinion','beautiful · ugly · cheap · expensive']
 };
 const cols=cats.map(cat=>{const [icon,label,hint]=meta[cat];const accepted=data.sort.map((x,i)=>({x,i})).filter(({i})=>state.sortPlaced[i]===cat);return `<div class="sortCol ${state.sortSelected!==null?'awaiting':''}" data-col="${cat}"><div class="sortColHead"><span class="sortColIcon">${icon}</span><div><h3>${label}</h3><small>${hint}</small></div><b>${accepted.length}/4</b></div><div class="sortDrop">${accepted.map(({x})=>`<span class="sortChip good">${esc(x.w)}</span>`).join('')||`<div class="dropPlaceholder">${state.sortSelected!==null?'Нажми сюда':'Перетащи слово сюда'}</div>`}</div></div>`}).join('');
 app.innerHTML=shell(`${title(3,'Garage Sort','Распредели слова по трём категориям')}<div class="blockBody"><div class="visualCard">${visualSlot('block3-sort.jpg','🧰','Garage Sort','Drag or tap words into three groups','Automotive design / sorting scene')}</div><div class="questionCard garageSortQuestion"><div class="kicker">Garage Sort</div><div class="prompt">Sort all 12 words.</div><div class="subprompt">Слова перемешаны. Перетащи карточку или выбери её и нажми нужную вертикальную категорию.</div><div class="sortWrap"><div class="sortBankHead"><strong>WORDS TO SORT</strong><span>${remaining.length} осталось</span></div><div class="sortBank" id="sortBank">${chips||'<div class="sortComplete">✓ Все слова распределены</div>'}</div><div class="sortColumns">${cols}</div></div><div class="statusWrap"><div id="fb">${placed===total?feedback('good','Отлично! Все 12 слов на своих местах.'):feedback('neutral',state.sortSelected!==null?'Теперь выбери подходящую категорию.':'Выбери слово или перетащи карточку.')}</div><div class="internalProgress"><div class="miniDots">${Array.from({length:total},(_,i)=>`<i class="miniDot ${i<placed?'done':i===placed?'current':''}"></i>`).join('')}</div><div class="counter">Распределено ${placed} из ${total}</div></div></div></div></div><div class="footerActions"><div class="leftActions">${audioBtn('allWords','Все слова')}</div><button class="nextBtn" id="next" ${placed===total?'':'disabled'}>Следующий блок →</button></div>`);
 document.getElementById('allWords').onclick=function(){playSequence(state.sortOrder.map(i=>data.sort[i].w),this,'▶ Все слова')};
 function place(i,cat){const item=data.sort[i];if(!item||state.sortPlaced[i])return;if(item.c===cat){recordAttempt('sort',i,true);state.sortPlaced[i]=cat;state.sortSelected=null;save();garageSort()}else{recordAttempt('sort',i,false);state.sortSelected=i;save();const fb=document.getElementById('fb');fb.innerHTML=feedback('bad','Эта категория не подходит. Карточка остаётся выбранной — попробуй другую.') }}
 document.querySelectorAll('[data-chip]').forEach(el=>{el.onclick=()=>{state.sortSelected=+el.dataset.chip;save();garageSort()};el.ondragstart=e=>{e.dataTransfer.setData('text/plain',el.dataset.chip);e.dataTransfer.effectAllowed='move'}});
...
function drive(){
 ...
 const routeOrder=[0,1,2,3,7,6,5,4,8,9,10,11,15,14,13,12];
 const cells=routeOrder.map((pos,slot)=>`<div class="roadCell ${pos===0?'start':''} ${pos===15?'finish':''} ${state.drive.pos===pos?'current':''}" style="--slot:${slot}"><span class="roadNumber">${pos===0?'START':pos===15?'FINISH':pos+1}</span>${state.drive.pos===pos?'<span class="carToken" aria-label="current position">🚗</span>':''}</div>`).join('');
 const canCheck=!doneAll&&!state.drive.canRoll&&chosen.length===item.tokens.length;
 app.innerHTML=shell(`${title(8,'Test Drive Challenge','Кубик · маршрут · sentence building')}<div class="driveGame"><section class="driveBoard"><div class="driveBoardHead"><div><span class="driveEyebrow">TEST DRIVE ROUTE</span><strong>Drive to the finish</strong></div><span class="routeProgress">${Math.min(step,data.drive.length)}/${data.drive.length} checkpoints</span></div><div class="road">${cells}</div><div class="dicePanel"><div class="diceFace"><span>${diceFaces[(state.drive.dice||1)-1]}</span><small>${state.drive.canRoll?'YOUR TURN':'MOVE +' + (state.drive.dice||1)}</small></div><div class="diceCopy"><strong>${state.drive.canRoll?'Брось кубик':'Машина передвинулась'}</strong><span>${state.drive.canRoll?'Начни новый checkpoint.':'Теперь собери фразу справа.'}</span></div><button class="rollBtn" id="roll" ${(!state.drive.canRoll||doneAll)?'disabled':''}>🎲 Бросить кубик</button></div></section><section class="driveChallenge"><div class="checkpointTop"><div><span class="driveEyebrow">CHECKPOINT ${Math.min(step+1,data.drive.length)}/${data.drive.length}</span><div class="checkpointTitle">${doneAll?'Финиш!':esc(item.cue)}</div></div><div class="checkpointFlag">${doneAll?'🏁':'🚘'}</div></div>${doneAll?`<div class="driveFinish"><span>🏁</span><strong>Маршрут завершён!</strong><p>Все 8 checkpoints пройдены. Отличная работа.</p></div>`:`<div class="sentenceStage"><div class="stageLabel"><span>YOUR SENTENCE</span><small>Нажми выбранное слово, чтобы убрать его</small></div><div class="driveAnswer">${chosen.map((x,i)=>`<button class="token answerToken" data-remove="${i}">${esc(x.t)}</button>`).join('')||'<span class="answerPlaceholder">После броска кубика собери фразу здесь.</span>'}</div><div class="stageLabel tokensLabel"><span>WORD CARDS</span><small>${state.drive.canRoll?'Сначала брось кубик':'Выбирай слова по порядку'}</small></div><div class="driveTokens">${shuffled.map(x=>`<button class="token ${chosen.some(c=>c.i===x.i)?'used':''}" data-token="${x.i}" ${state.drive.canRoll?'disabled':''}>${esc(x.t)}</button>`).join('')}</div><div class="driveActions"><button class="smallBtn" id="clearDrive" ${state.drive.canRoll||!chosen.length?'disabled':''}>Сбросить фразу</button><button class="smallBtn primary" id="checkDrive" ${canCheck?'':'disabled'}>Проверить →</button></div></div>`}<div class="driveStatus"><div id="fb">${doneAll?feedback('good','Все 8 checkpoints пройдены!'):feedback('neutral',state.drive.canRoll?'Брось кубик слева, чтобы начать checkpoint.':'Собери фразу и нажми «Проверить».')}</div><div class="internalProgress"><div class="miniDots">${Array.from({length:data.drive.length},(_,i)=>`<i class="miniDot ${i<step?'done':i===step?'current':''}"></i>`).join('')}</div><div class="counter">Checkpoint ${Math.min(step,data.drive.length)} из ${data.drive.length}</div></div></div></section></div><div class="footerActions"><div class="leftActions">${doneAll?'':audioBtn('driveModel','Модель')}</div><button class="nextBtn" id="next" ${doneAll?'':'disabled'}>Результаты →</button></div>`);
 ...
 document.getElementById('clearDrive').onclick=()=>{state.drive.chosen=[];save();drive()};
 document.getElementById('checkDrive').onclick=()=>{if(!canCheck)return;const answer=chosen.map(x=>x.t).join(' '),target=item.tokens.join(' '),ok=answer===target;recordAttempt(sec,step,ok);if(ok){playText(item.audio,null);state.drive.step=step+1;state.drive.canRoll=true;state.drive.chosen=[];save();drive()}else{document.getElementById('fb').innerHTML=feedback('bad','Порядок пока неверный. Нажми нужное слово в собранной фразе, убери его и попробуй снова.')}};
}

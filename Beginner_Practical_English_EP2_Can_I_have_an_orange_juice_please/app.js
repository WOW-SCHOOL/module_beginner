...
function providerUrls(text){const clean=text.replace(/\s+/g,' ').trim();const q=encodeURIComponent(clean),len=clean.length;return [
  `https://translate.google.com/translate_tts?ie=UTF-8&client=tw-ob&tl=en-GB&ttsspeed=0.9&total=1&idx=0&textlen=${len}&q=${q}`,
  `https://translate.google.co.uk/translate_tts?ie=UTF-8&client=tw-ob&tl=en-GB&ttsspeed=0.9&total=1&idx=0&textlen=${len}&q=${q}`,
...
function voiceScore(v){const n=(v.name||'').toLowerCase(),l=(v.lang||'').toLowerCase();let s=0;if(l==='en-gb')s+=60;else if(l.startsWith('en-gb'))s+=55;else if(l.startsWith('en'))s+=15;if(/natural|online|neural/.test(n))s+=35;if(/sonia|libby|ryan|hazel|george/.test(n))s+=30;if(/google uk english|google.*english.*uk/.test(n))s+=40;if(/female/.test(n))s+=4;return s}
async function getBestBritishVoice(){if(!window.speechSynthesis)return null;let voices=speechSynthesis.getVoices();if(!voices.length){await new Promise(resolve=>{let done=false;const finish=()=>{if(done)return;done=true;resolve()};speechSynthesis.addEventListener?.('voiceschanged',finish,{once:true});setTimeout(finish,700)});voices=speechSynthesis.getVoices()}return voices.sort((a,b)=>voiceScore(b)-voiceScore(a))[0]||null}
async function speakBrowserChunk(text,token,voice){if(token!==playToken)throw new Error('cancelled');if(!window.speechSynthesis||!window.SpeechSynthesisUtterance)throw new Error('no-speech');return new Promise((resolve,reject)=>{const u=new SpeechSynthesisUtterance(text);u.lang='en-GB';u.rate=.92;u.pitch=1;u.volume=1;if(voice)u.voice=voice;let done=false;const timer=setTimeout(()=>{if(done)return;done=true;try{speechSynthesis.cancel()}catch(e){}reject(new Error('speech-timeout'))},18000);u.onend=()=>{if(done)return;done=true;clearTimeout(timer);resolve()};u.onerror=()=>{if(done)return;done=true;clearTimeout(timer);reject(new Error('speech-error'))};speechSynthesis.speak(u)})}
async function playBrowserSequence(chunks,startIndex,token){const voice=await getBestBritishVoice();for(let i=startIndex;i<chunks.length;i++){if(token!==playToken)return;await speakBrowserChunk(chunks[i],token,voice);await new Promise(r=>setTimeout(r,90))}}
async function playSequence(texts,btn,idle='▶ Прослушать'){stopAudio();const token=++playToken;btn?.classList.add('busy');btn?.classList.remove('error');if(btn)btn.textContent='■ Стоп';const chunks=texts.flatMap(raw=>splitTTS(raw));let i=0;try{for(;i<chunks.length;i++){if(token!==playToken)return;try{await playChunk(chunks[i],token)}catch(netErr){try{ttsAudio.pause()}catch(e){}await playBrowserSequence(chunks,i,token);i=chunks.length;break}}if(token===playToken){btn?.classList.remove('busy');btn.textContent=idle}}catch(e){if(token===playToken){btn?.classList.remove('busy');btn?.classList.add('error');btn.textContent='Не удалось воспроизвести';setTimeout(()=>{btn?.classList.remove('error');btn.textContent=idle},1800)}}}

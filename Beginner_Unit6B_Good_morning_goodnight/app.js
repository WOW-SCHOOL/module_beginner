...
function commonQuestionScreen(sec,blockNum,heading,sub,visualHTML,item,total,audioText,afterLast,customAudio=null,helpers=[]){
  ...
  app.innerHTML=shell(`${title(blockNum,heading,sub)}<div class="blockBody"><div class="visualCard">${visualHTML}</div><div class="questionCard"><div class="kicker">${LABELS[sec]}</div><div class="prompt" style="white-space:pre-line">${esc(item.q)}</div><div class="subprompt">Выбери правильный вариант. Если сложно, открой текст или подсказку ниже.</div><div class="answers">${options.map((o,oi)=>`<button class="answer" data-answer="${oi}" data-value="${esc(o)}" ${done?'disabled':''}>${esc(o)}</button>`).join('')}</div><div class="helpersStack">${helperDetails(helpers)}</div><div class="statusWrap"><div id="fb">${done?feedback('good','Верно!'):feedback('neutral','Ответ не выбран')}</div>${miniProgress(sec,idx,total)}</div></div></div><div class="footerActions"><div class="leftActions">${audioText?audioBtn('audio',customAudio||'Прослушать'):''}</div><button class="nextBtn" id="next" ${done?'':'disabled'}>${idx===total-1?'Следующий блок →':'Следующее задание →'}</button></div>`);
...
function start(){
  app.innerHTML=shell(`<div class="hero"><div><div class="eyebrow">WOW SCHOOL · Interactive lesson module</div><h1>Good morning, <span>goodnight</span></h1><p class="lede">В этом модуле мы тренируем <b>adverbs of frequency</b>, лексику по теме <b>a typical day</b> и произношение <b>/j/ / juː/</b>. Я добавил скрытые <b>текстовые подсказки</b> там, где ученику может быть трудно на слух, а также лёгкие правила и hints в сложных местах.</p><div class="heroFeatures"><span>Morning routine</span><span>Listening + transcript</span><span>Hints & mini rules</span><span>Experimental block 8</span></div><div class="heroBtns"><button class="btn primary" id="startBtn">Начать →</button><button class="btn secondary" id="reset">Сбросить прогресс</button></div><div class="devNote">Скрытый режим разработчика: двойной клик по бейджу Unit 6B или <b>Ctrl + Alt + D</b>.</div></div><div class="heroVisual">${heroSlot()}</div></div>`);
...
function warmup(){
  ...
  commonQuestionScreen(sec,1,'Morning Warm-up','easy daily routine phrases · listen and choose',visualSlot('block1-warmup.jpg','🌅','Morning Warm-up','Start with very easy daily routine phrases','Morning routine and breakfast scene'),item,data.warmup.length,item.audio,()=>{state.screen=2;save();render()},'Фраза',helpers);
}
...
function listen1(){
  ...
  commonQuestionScreen(sec,2,'Morning Person','easy interview · audio + hidden transcript',visualSlot('block2-listening.jpg','🎧','Morning Person','Listen to Hannah and answer simple questions','Daily routine listening scene'),item,data.listen1.qs.length,data.listen1.transcript.join(' '),()=>{state.screen=3;save();render()},'Интервью',helpers);
}
...
function pron(){
  ...
  commonQuestionScreen(sec,6,'Pronunciation','/j/ · /juː/',visualSlot('block6-pronunciation.jpg','🗣️','Pronunciation','Listen carefully: yes / use / usually / you','Pronunciation practice'),item,data.pron.length,item.audio,()=>{state.screen=7;save();render()},'Аудио',helpers)
}
...
function listen2(){
  ...
  app.innerHTML=shell(`${title(7,'Mini Dialogues','short routine dialogues · transcript support')}<div class="blockBody"><div class="visualCard">${visualSlot('block7-listening.jpg','🎧','Mini Dialogues','Listen to short everyday dialogues','Daily routine listening scene')}</div><div class="questionCard"><div class="kicker">Dialogue ${script.id}</div><div class="prompt">${esc(item.q)}</div><div class="subprompt">Диалог можно прослушивать несколько раз. Если трудно, открой скрытый текст.</div><div class="answers">${options.map(o=>`<button class="answer" data-value="${esc(o)}" ${done?'disabled':''}>${esc(o)}</button>`).join('')}</div><div class="helpersStack">${helperDetails([{title:'Открыть текст диалога',html:transcriptHtml(script.lines.map(x=>x.text))},{title:'Подсказка',html:listHint(['Сначала слушай вопрос, затем лови ключевые слова в ответе.', 'Обращай внимание на time words: at seven, at six, at about eleven.'])}])}</div><div class="statusWrap"><div id="fb">${done?feedback('good','Верно!'):feedback('neutral','Сначала прослушай диалог')}</div>${miniProgress(sec,idx,data.listen2.qs.length)}</div></div></div><div class="footerActions"><div class="leftActions">${audioBtn('listenAudio',`Диалог ${script.id}`)}</div><button class="nextBtn" id="next" ${done?'':'disabled'}>${idx===data.listen2.qs.length-1?'Следующий блок →':'Следующее задание →'}</button></div>`);
...
function error(){
  ...
  app.innerHTML=shell(`${title(8,'Routine Repair','find the mistake → fix it → hear the model')}<div class="blockBody"><div class="visualCard">${visualSlot('block8-experimental.jpg','🛠️','Routine Repair','Find the wrong word and repair the sentence','Learners checking daily routine sentences')}</div><div class="questionCard errorQuestion"><div class="kicker">Routine Repair</div><div class="prompt">Fix the sentence.</div><div class="subprompt">Нажми на слово с ошибкой. Затем выбери правильную замену. Это блок-эксперимент с быстрой грамматической коррекцией.</div><div class="errorWork"><div class="errorSentence ${done?'solved':''}">${words.map((t,i)=>`<button class="errorWord ${done&&i===item.err?'correct':''} ${es.selected===i&&!done?'selected':''}" data-token="${i}" ${done?'disabled':''}>${esc(t)}</button>`).join('')}</div><div class="replacementArea ${optionsVisible?'show':''}">${done?'<div class="replaceHint doneHint">Готово. Правильное предложение можно прослушать ещё раз.</div>':optionsVisible?`<div class="replaceLabel">На что заменить <b>${esc(item.tokens[item.err])}</b>?</div><div class="fixOptions">${item.options.map(o=>`<button class="fixChoice" data-fix="${esc(o)}">${esc(o)}</button>`).join('')}</div>`:'<div class="replaceHint">Выбери слово с ошибкой — варианты замены появятся здесь.</div>'}</div></div><div class="helpersStack">${helperDetails([{title:'Подсказка / правило',html:`<p>${esc(item.hint)}</p>`}])}</div><div class="statusWrap"><div id="fb">${done?feedback('good','Исправлено! Правильное предложение уже озвучено.'):feedback('neutral',es.message||'Найди слово с ошибкой.')}</div>${miniProgress(sec,idx,total)}</div></div></div><div class="footerActions"><div class="leftActions"><button class="audioBtn" id="errorModel" ${done?'':'disabled'}>▶ Модель</button><span class="audioMeta">British English · синтез речи</span></div><button class="nextBtn" id="next" ${done?'':'disabled'}>${idx===total-1?'Результаты →':'Следующее задание →'}</button></div>`);

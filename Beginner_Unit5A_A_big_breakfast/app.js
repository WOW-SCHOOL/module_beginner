...
function transcriptHTML(lines=[]){
 return `<div class="transcript">${lines.map((line,i)=>`<div class="transcriptLine"><strong>${i%2===0?'Interviewer':'Guest'}</strong><span>${esc(line.text||'')}</span></div>`).join('')}</div>`;
}
